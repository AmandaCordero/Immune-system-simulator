...
def recocido_simulado(param_inicial,serotipo, temp_inicial=1.0, temp_final=1e-2, alpha=0.9, max_iter=100):
    estado_actual = param_inicial
    res = sim(estado_actual, 48)
    energia_actual = metrica(res, serotipo)
    T = temp_inicial
    mejor_estado = estado_actual
    mejor_energia = energia_actual
    i = 0
    with open(f"log_sim.txt", "a") as f:
            f.write(f"serotipo: {serotipo}\n")
            f.write(f"corrida {i}: {param_inicial}\n")
            f.write(f"energia {i}: {energia_actual}\n")
    ...
    while T > temp_final and i < max_iter:
        ...
        energia_nueva = metrica(resn, serotipo)
        with open(f"log_sim.txt", "a") as f:
            f.write(f"corrida {i}: {estado_nuevo}\n")
            f.write(f"energia {i}: {energia_nueva}\n")
...
def metrica(antibodies, serotipo):
    ...
    pre_vals = df[serotipos[serotipo][0]].values.reshape(-1, 1)
    post_vals = df[serotipos[serotipo][1]].values.reshape(-1, 1)
    
    points1 = [(pre_vals[i], post_vals[i]) for i in range(len(pre_vals))]
    points2 = [(antibodies[i][serotipo][0], antibodies[i][serotipo][1]) for i in range(len(antibodies))]

    distancias_chamfer = chamfer_distance(points1, points2)

    return distancias_chamfer
...
if __name__ == "__main__":
    ...
    for s in serotipos.keys():
        print(f"Serotipo {s}")
        a = recocido_simulado(SIMULATION_PARAMS, s)
        with open(f"log_sim.txt", "a") as f:
            f.write(f"serotipo: {s}\n")
            f.write(f"best params: {a[0]}\n")
            f.write(f"best energy: {a[1]}\n")
        ...
        for ser in serotipos.keys():
            dictio[ser] = []
        for elem in lista:
            for se in serotipos.keys():
                dictio[se].append(elem[se])

        with open(f"log_simulacion{s}.txt", "w") as f:
            f.write(f"{dictio}")
